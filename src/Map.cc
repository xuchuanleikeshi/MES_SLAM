...
void Map::AddSparsifiedMapPoint(MapPoint* pMP) {
    // mspSparsifiedMapPoints 是 std::set<MapPoint*> 类型，直接插入裸指针
    mspSparsifiedMapPoints.insert(pMP);
}

void Map::AddSparsifiedKeyFrame(KeyFrame* pKF) {
    unique_lock<mutex> lock(mMutexMap);

    // mspSparsifiedKeyFrames 是 std::set<KeyFrame*> 类型，直接插入裸指针
    mspSparsifiedKeyFrames.insert(pKF);

    // 获取与 KeyFrame 相关联的 MapPoints
    vector<MapPoint*> vMPs = pKF->GetMapPointMatches();
    for (MapPoint* pMPi : vMPs) {
        // 使用裸指针调用 AddSparsifiedMapPoint
...
//获取稀疏特征点
set<MapPoint*> Map::GetSparsifiedMapPoints()
{
    unique_lock<mutex> lock(mMutexMap);
    return mspSparsifiedMapPoints;
}
