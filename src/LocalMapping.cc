...
void LocalMapping::Run()
{
    ...
    while(1)
    {
        ...
        if(CheckNewKeyFrames())
        {
            ...
            // 加入关键帧（注意线程安全）
            {
                std::unique_lock<std::mutex> lock(mMutexKeyFramesToSparsification);
                mlpKeyFramesToSparsification.push_back(mpCurrentKeyFrame);  // mpCurrentKeyFrame 是 KeyFrame*
            }

            // 处理稀疏化关键帧集合
            {
                std::unique_lock<std::mutex> lock(mMutexKeyFramesToSparsification);
                for (auto lit = mlpKeyFramesToSparsification.begin(); lit != mlpKeyFramesToSparsification.end(); ) {
                    KeyFrame* pKFi = *lit;

                    // 注意加 nullptr 判空，防止外部线程已删或未初始化
                    if (!pKFi || pKFi->isBad()) {
                        ...
                        if (pKFi->mnBALocalForKF != mpCurrentKeyFrame->mnId &&
                            ...
                            if (bToSparse) {
                                mpMapSparsification->InsertKeyFrame(pKFi);  // 这里也要接受裸指针
                                lit = mlpKeyFramesToSparsification.erase(lit);
                            } else {
                                ++lit;
                            }
        ...
        //===========稀疏化的准备工作，准备关键帧集合，利用冗余机制进行选择==================
        // mlpKeyFramesToSparsification.push_back(mpCurrentKeyFrame);  // mpCurrentKeyFrame 是 KeyFrame*
        //
        // for (std::list<KeyFrame*>::iterator lit = mlpKeyFramesToSparsification.begin(), lend = mlpKeyFramesToSparsification.end();lit != lend;) {
        //     KeyFrame* pKFi = *lit;
        //     if (pKFi->isBad()) {
        //         lit = mlpKeyFramesToSparsification.erase(lit);  // 只从列表中移除，不 delete
        //     } else {
        //         if (pKFi->mnBALocalForKF != mpCurrentKeyFrame->mnId &&
        //             pKFi->mnBAFixedForKF != mpCurrentKeyFrame->mnId) {
        //             bool bToSparse = pKFi->UpdateCountInLocalMapping(false);
...
void LocalMapping::SetMapSaprsification(MapSparsification *pMapSparsification) {
