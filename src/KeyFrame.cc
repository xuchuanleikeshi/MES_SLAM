...
KeyFrame::KeyFrame(Frame &F, Map *pMap, KeyFrameDatabase *pKFDB):
    ...
    mpMap(pMap),mnMapSaprsificationId(0),mnCountInLocal(0),mbNonLocalKF(false)
{
    // 获取id
    mnId=nNextId++;
...
//                 vpMapPointsNew.push_back(pMP);
//                 pMP->UpdateObservation(pThis, nGoodMPS);
//                 nGoodMPS++;
//             }
//         }
//
...
//
//         // MapPoints交换
//         vector<MapPoint*> oldMapPoints;
//         oldMapPoints.swap(mvpMapPoints);
...
//             const float invWidth = FRAME_GRID_COLS / static_cast<float>(mnMaxX - mnMinX);
//             const float invHeight = FRAME_GRID_ROWS / static_cast<float>(mnMaxY - mnMinY);
//
//             // 初始化网格
//             mGrid = vector<vector<vector<size_t>>>(FRAME_GRID_COLS,
//                  vector<vector<size_t>>(FRAME_GRID_ROWS));
//
//             // 重新分配特征点到网格
//             for(size_t i = 0; i < mvKeysUn.size(); i++) {
//                 const cv::KeyPoint &kp = mvKeysUn[i];
//                 int posX = round((kp.pt.x - mnMinX) * invWidth);
//                 int posY = round((kp.pt.y - mnMinY) * invHeight);
//
//                 // 确保在网格范围内
//                 posX = max(0, min(FRAME_GRID_COLS-1, posX));
//                 posY = max(0, min(FRAME_GRID_ROWS-1, posY));
//
...
//         vector<cv::KeyPoint>().swap(mvKeysRight);
//         mbSparsified = true;
//     }
// }
...
void KeyFrame::EraseBadDescriptor() {
...

    {
        ...
        // 7. 清理和兼容处理
        // 注意：mvKeys 不能清空，UnprojectStereo 使用它
        mvKeys = mvKeysUn;  // 保证 UnprojectStereo 使用的是有效关键点
        vector<cv::KeyPoint>().swap(mvKeysRight);  // 可以清空右目
        mbSparsified = true;
    }
}
