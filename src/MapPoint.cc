...
MapPoint::MapPoint(const cv::Mat &Pos,  //地图点的世界坐标
    ...
    mfMaxDistance(0),                       //上界
    mpMap(pMap),mnMapSparsificationId(0),mnIndexForSparsification(0)                             //从属地图
{
...
MapPoint::MapPoint(const cv::Mat &Pos, Map* pMap, Frame* pFrame, const int &idxF):
    ...
    mnFound(1), mbBad(false), mpReplaced(NULL), mpMap(pMap),mnMapSparsificationId(0),mnIndexForSparsification(0)
{
    Pos.copyTo(mWorldPos);
...
void MapPoint::UpdateObservation(KeyFrame* pKF, int idx)
{
    ...
    // If the KeyFrame already has an observation, get its index
    size_t observationIndex = 0;
    if (mObservations.count(pKF)) {
        observationIndex = mObservations[pKF];  // Existing observation index
    }

    // If KeyFrame has a left view, check for the index
    if (pKF->GetNLeft() != -1 && idx >= pKF->GetNLeft()) {
        observationIndex = idx;  // Update observation index if condition met
    } else {
        observationIndex = idx;  // Update observation index if condition met
    }

    // Store the updated observation index in the map
    mObservations[pKF] = observationIndex;
}
