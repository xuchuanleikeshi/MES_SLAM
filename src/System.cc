        			 ...
        			 mbDeactivateLocalizationMode(false)		//没有这个模式转换标志
{
    ...
    mpLocalMapper = new LocalMapping(mpMap,  mSensor==MONOCULAR);	// TODO 为什么这个要设置成为MONOCULAR？？？
    //运行这个局部建图线程
    mptLocalMapping = new thread(&ORB_SLAM2::LocalMapping::Run,mpLocalMapper);				//这个调用函数的参数

    //Initialize the Loop Closing thread and launchiomanip
    mpLoopCloser = new LoopClosing(mpMap, 	mpKeyFrameDatabase,  mpVocabulary, mSensor!=MONOCULAR);			//当前的传感器是否是单目
    //创建回环检测线程
    mptLoopClosing = new thread(&ORB_SLAM2::LoopClosing::Run,mpLoopCloser);					//该函数的参数


    mpMapSaprsification = new MapSparsification(strSettingsFile, mpMap);
    mptMapSparsification = new thread(&ORB_SLAM2::MapSparsification::Run, mpMapSaprsification);
    mpMapSaprsification->SetLoopClosing(mpLoopCloser);
    KeyFrame::mnNonLocalKF = fsSettings["Sparsification.NonLocalKF"];
    ...
    mpLocalMapper->SetMapSaprsification(mpMapSaprsification);
...
int System::ReBacktheMappoint() {
    if (!mpMap) return 0;  // 避免空指针访问
    return mpMap->GetAllMapPoints().size()-mpMap->GetSparsifiedMapPoints().size();  // 直接返回 size() 避免遍历
}
